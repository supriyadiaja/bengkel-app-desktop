...
const api = window.api || { send: () => {} };

function TitleBar() {
  const [maximized, setMaximized] = useState(false);

  const handleMinimize = () => api.send('window:minimize');
  const handleMaximize = () => {
    api.send('window:maximize');
    setMaximized(m => !m);
  };
  const handleClose = () => api.send('window:close');
  ...
  return (
    <div style={{
      ...
      // Drag area agar window bisa dipindah
      WebkitAppRegion: 'drag',
      flexShrink: 0,
      paddingRight: 0,
    }}>
      {/* Tombol window controls - no-drag */}
